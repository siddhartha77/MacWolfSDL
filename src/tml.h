...
static int tml_stream_memory_read(void* s, void* ptr, unsigned int size)
{
	struct tml_stream_memory* m = s;
...
TMLDEF tml_message* tml_load(struct tml_stream* stream)
{
	...
	if (p.message_count)
	{
		...
		double ticks = 0, tempo_ticks = 0; //tick counter and value at last tempo change
		double step_smallest, msec, tempo_msec = 0; //msec value at last tempo change
